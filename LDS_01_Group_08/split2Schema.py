...
def makeMatchRow(rowIn, csvOut, keySet):
    ...
    for feat,type in MATCH_FEAT_TYPE.items():
        ...
        elif feat == 'tourney_id':
            tourneyId = str(rowIn['tourney_id']) + '-' + str(rowIn['tourney_level'])
            rowOut.append(tourneyId)
        else:
            try:
...
def splitAll(pathIn, pathOutList):
...
prev = datetime.now()
splitAll(PATH_TENNIS, ALL_PATHS_TABS)
next = datetime.now()
print("Split time: %s ms" % str(next-prev))
